	...
	angular 
		...
		function ResidentController($scope, $stateParams, ResidentInfoService, $firebaseObject){
			...
			ResidentInfoService.getResidentInfo($stateParams.id).then(function(response){
				...
				console.log($scope.residentInfo)
				var ref = new Firebase(response.data.current_location_url)
				$scope.currentLocation = $firebaseObject(ref);
			});
...
angular.module("pcui").run(["$templateCache", function($templateCache) {$templateCache.put("app/chat/chat.TMPL.html","");
$templateCache.put("app/calendar/calendar.html","<md-content class=calendar-view ng-controller=CalendarController><div layout=row><div flex=25><md-button class=md-raised ng-click=addEvent()>Add Event</md-button><div class=sources ng-repeat=\"resident in residents\"><div class=sources-container ng-click=changeCalendar(resident.id)>{{resident.first_name + \' \' + resident.last_name}}</div></div></div><div flex=75 class=calendar ui-calendar=uiConfig.calendar calendar=myCalendar ng-model=events></div></div></md-content>");
...
//# sourceMappingURL=../maps/scripts/app-f1a813b01b.js.map
